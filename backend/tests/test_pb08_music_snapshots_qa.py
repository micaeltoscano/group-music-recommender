"""Testes independentes do QA para o PB-08 (coleta e cache de dados musicais).

Autoria do Agente de Teste (QA), separados dos testes do implementador
(`test_pb08_music_snapshots.py`). Foco adversarial: isolamento entre usuários,
fronteira exata do TTL, vazamento de token, persistência e adesão da marcação de
reauth, entradas inválidas e corrida na criação do snapshot.
