"""Testes de validação independente do QA para o PB-02 (Autenticação Spotify).

IMPORTANTE: estes são testes ADICIONADOS PELO AGENTE DE TESTE (QA), pois a entrega
do PB-02 não incluiu testes automatizados de autenticação. São claramente
identificados pelo sufixo `_qa` e cobrem os casos CT-PB02-01..07 do PLANO_TESTES.md.

- Nenhuma chamada real ao Spotify: o cliente externo é totalmente mockado.
- Nenhum token/segredo real é usado ou registrado.
"""
