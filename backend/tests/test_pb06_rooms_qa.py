"""Testes independentes do QA para o PB-06 (contexto e modo de consenso).

Estes testes são de autoria do Agente de Teste (QA), separados dos testes do
implementador (`test_pb06_rooms.py`). Objetivo: atacar de forma adversarial a
autorização (não-membro, não autenticado), os limites textuais, a semântica de
substituição do contexto e o comportamento em sala expirada — sem confiar no
relato do Dev.
"""
